...
public:
  ...
  bool MustSendCoopRts (void) const;
  /**
...
private:
  ...
  bool m_sendCoopRts; //!< send an COOPRTS?
...
public:
  struct CoopTable
  {
   ...
   Mac48Address HelperMacAddres;
   Mac48Address DestinationMacAddres;
   Mac48Address SourceMacAddres;
   float TimeOffHear;
   float Rhd;
   float Rsh;
   float Rsd;
   uint8_t NumOfFailures;

  };//cooptable structure
  typedef struct CoopTable CoopTable;
 std::list<CoopTable> m_CoopTable;
 TracedCallback<std::list<CoopTable>> m_cooptableCh;
  ...
  WifiMode WifiModeFromSnr(float rxSnr,uint16_t channelWidth,uint16_t guardIntervalh,uint16_t nStream );

  void OverHeard (Ptr<const Packet> packet,uint16_t channelFreqMhz,WifiTxVector txVector,MpduInfo aMpdu,SignalNoiseDbm signalNoise);
  void PrintCoopTable(std::list<CoopTable> cooptable);
  bool LookupCoopTable(std::list<CoopTable> cooptable, Mac48Address HelperMacAddress,Mac48Address DestinationMacAddres);
