...
void
MacLowTransmissionParameters::EnableCoopRts (void)
{
  m_sendCoopRts = true;
}
void
MacLowTransmissionParameters::DisableCoopRts (void)
{
  m_sendCoopRts = false;
}
...
bool
MacLowTransmissionParameters::MustSendCoopRts (void) const
{
  return m_sendCoopRts;
}
bool
...
MacLow::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::MacLow")
    ...
    .AddTraceSource ("CoopTable",
          "change cooptable",
          MakeTraceSourceAccessor (&MacLow::m_cooptableCh),
                     "ns3::MacLow::TracedCallback")
  ;
  return tid;
}
...
MacLow::SetPhy (const Ptr<WifiPhy> phy)
{
  ...
  m_phy->TraceConnectWithoutContext ("MonitorSnifferRx",MakeCallback (&MacLow::OverHeard, this));
  SetupPhyMacLowListener (phy);
}

WifiMode
MacLow::WifiModeFromSnr(float rxSnr,uint16_t channelWidth,uint16_t guardIntervalh,uint16_t nStream )
{
  WifiMode m_WifiMode;
  switch (nStream)
    {
    case 1:
      {
       switch (channelWidth)
         ...
         case 20:
           {
             if (rxSnr < 3.9) {m_WifiMode = WifiMode("DsssRate1Mbps");}
             if (rxSnr < 7 && rxSnr >=3.9) {m_WifiMode = WifiMode("HtMcs0");}
             if(rxSnr>=7 && rxSnr<9.9){m_WifiMode = WifiMode("HtMcs1");}
             if(rxSnr>=9.9 && rxSnr<13.7){m_WifiMode = WifiMode("HtMcs2");}
             if(rxSnr>=13.7 && rxSnr<16.6){m_WifiMode = WifiMode("HtMcs3");}
             if(rxSnr>=16.6 && rxSnr<21){m_WifiMode = WifiMode("HtMcs4");}
             if(rxSnr>=21 && rxSnr<23){m_WifiMode = WifiMode("HtMcs5");}
             if(rxSnr>=23 && rxSnr<24){m_WifiMode = WifiMode("HtMcs6");}
             if(rxSnr>=24 ){m_WifiMode = WifiMode("HtMcs7");}
            }
             break;
       ...
       switch (channelWidth)
         {
          case 20:
           {
            if (rxSnr < 3.9) {m_WifiMode = WifiMode("DsssRate1Mbps");}
            if (rxSnr < 7 && rxSnr >=3.9) {m_WifiMode = WifiMode("HtMcs8");}
            if(rxSnr>=7 && rxSnr<9.9){m_WifiMode = WifiMode("HtMcs9");}
            if(rxSnr>=9.9 && rxSnr<13.7){m_WifiMode = WifiMode("HtMcs10");}
            if(rxSnr>=13.7 && rxSnr<16.6){m_WifiMode = WifiMode("HtMcs11");}
            if(rxSnr>=16.6 && rxSnr<21){m_WifiMode = WifiMode("HtMcs12");}
            if(rxSnr>=21 && rxSnr<23){m_WifiMode = WifiMode("HtMcs13");}
            if(rxSnr>=23 && rxSnr<24){m_WifiMode = WifiMode("HtMcs14");}
            if(rxSnr>=4 ){m_WifiMode = WifiMode("HtMcs15");}
       ...
       switch (channelWidth)
        {
         case 20:
          {
           if (rxSnr < 3.9) {m_WifiMode = WifiMode("DsssRate1Mbps");}
           if (rxSnr < 7 && rxSnr >=3.9) {m_WifiMode = WifiMode("HtMcs16");}
           if(rxSnr>=7 && rxSnr<9.9){m_WifiMode = WifiMode("HtMcs17");}
           if(rxSnr>=9.9 && rxSnr<13.7){m_WifiMode = WifiMode("HtMcs18");}
           if(rxSnr>=13.7 && rxSnr<16.6){m_WifiMode = WifiMode("HtMcs19");}
           if(rxSnr>=16.6 && rxSnr<21){m_WifiMode = WifiMode("HtMcs20");}
           if(rxSnr>=21 && rxSnr<23){m_WifiMode = WifiMode("HtMcs21");}
           if(rxSnr>=23 && rxSnr<24){m_WifiMode = WifiMode("HtMcs22");}
           if(rxSnr>=24 ){m_WifiMode = WifiMode("HtMcs23");}
          }
...
bool
MacLow::LookupCoopTable(std::list<CoopTable> cooptable, Mac48Address helperMacaddress,Mac48Address destinationMacaddres)
{
 std::list<CoopTable>::iterator listIterator;
  for (listIterator=cooptable.begin(); listIterator!=cooptable.end(); listIterator++)
    {
    if (listIterator->HelperMacAddres == helperMacaddress && listIterator->DestinationMacAddres==destinationMacaddres)
      {
        return true;
      }
    }
...
void
MacLow::PrintCoopTable(std::list<CoopTable> cooptable)
{
  std::list<CoopTable>::iterator listIterator;
...
void
MacLow::OverHeard (Ptr<const Packet> packet,uint16_t channelFreqMhz,WifiTxVector txVector,MpduInfo aMpdu,SignalNoiseDbm signalNoise)
 {
  Mac48Address m_destinationMacAddress;
  Mac48Address m_helperMacAddress;
  Mac48Address m_sourceMacAddress;
  WifiMacHeader hdr;
  packet->PeekHeader(hdr);
  m_destinationMacAddress = hdr.GetAddr1 ();
  m_helperMacAddress = hdr.GetAddr2 ();
  m_sourceMacAddress = this->GetAddress ();
  WifiTxVector m_wifiTxVector;
  WifiMode m_wifiMode;
  m_wifiMode = txVector.GetMode();
  if(hdr.GetAddr1 ()== m_sourceMacAddress||hdr.GetAddr2 ()== m_sourceMacAddress||hdr.GetAddr1 ()=="ff:ff:ff:ff:ff:ff")
    {
      return;
    }
  if(hdr.IsData()&& m_wifiMode.GetDataRate(txVector)/10e+5>1)
   {
    float Rhd= m_wifiMode.GetDataRate(txVector)/10e+5;
    m_wifiMode= WifiModeFromSnr(signalNoise.signal- signalNoise.noise,txVector.GetChannelWidth(),txVector.GetGuardInterval(),txVector.GetNss());
    float Rsh= m_wifiMode.GetDataRate(txVector)/10e+5;
    hdr.SetAddr1 (m_destinationMacAddress);
    hdr.SetAddr2 (m_sourceMacAddress);
    m_wifiTxVector=GetDataTxVector (packet,&hdr);
    m_wifiMode=m_wifiTxVector.GetMode ();
    float Rsd= m_wifiMode.GetDataRate(txVector)/10e+5;
    if(Rsd >= Rhd || Rsd==1 || Rhd+Rsh<4*Rsd)
     {
      return;
     }
    if(!LookupCoopTable (m_CoopTable,m_helperMacAddress,m_destinationMacAddress))
     {
      CoopTable cooptable;
      cooptable.DestinationMacAddres=m_destinationMacAddress;
      cooptable.HelperMacAddres=m_helperMacAddress;
      cooptable.SourceMacAddres=m_sourceMacAddress;
      cooptable.TimeOffHear=Simulator::Now ().GetSeconds ();
      cooptable.Rsh=Rsh;
      cooptable.Rsd=Rsd;
      cooptable.Rhd=Rhd;
      cooptable.NumOfFailures=0;
      this->m_CoopTable.push_back (cooptable);
      std::cout<<"m_CoopTable.push_back"<<this->GetAddress ();
      m_cooptableCh(m_CoopTable);

       PrintCoopTable (m_CoopTable);
      }
